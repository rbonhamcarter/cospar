    ...
    * If `later_time_point` is specified, the function produces a map
    ...
    if (
        ...
        if not (
            ...
            similarity_matrix_full = generate_similarity_matrix(
                adata_orig,
    ...
    if use_full_Smatrix:
        ...
        for round_of_smooth in smooth_array:
            if not os.path.exists(similarity_file_name + f"_SM{round_of_smooth}.npz"):
                ...
                logg.error(
                    f"Please re-run the function with: compute_new=True. If you want to use smooth round not the multiples of 5, set save_subset=False"
                )
                return None
    ...
    if "Smatrix" not in adata.uns.keys():
        ...
        for round_of_smooth in smooth_array:
            ...
            if use_full_Smatrix:
                ...
                initial_similarity = generate_initial_similarity(
                    similarity_matrix_full_sp, clonal_cell_id_t1, clonal_cell_id_t1
                )
                final_similarity = generate_final_similarity(
                    similarity_matrix_full_sp, clonal_cell_id_t2, clonal_cell_id_t2
                )

                if extend_Tmap_space:
                    initial_similarity_ext = generate_initial_similarity(
                        similarity_matrix_full_sp, Tmap_cell_id_t1, clonal_cell_id_t1
                    )
                    final_similarity_ext = generate_final_similarity(
...
def infer_intraclone_Tmap(adata, intraclone_threshold=0.05, normalization_mode=1):
    ...
    if "transition_map" not in adata.uns.keys():
        ...
        demultiplexed_map = refine_Tmap_through_cospar_noSmooth(
            multiTime_cell_id_t1,
            multiTime_cell_id_t2,
            proportion,
            transition_map,
...
def infer_Tmap_from_one_time_clones_twoTime(
    ...
    if N_valid_time != 2:
        ...
        if initialize_method == "OT":
            ...
            infer_Tmap_from_optimal_transport(
                adata,
                OT_epsilon=OT_epsilon,
                OT_cost=OT_cost,
                OT_dis_KNN=OT_dis_KNN,
        ...
        if joint_optimization:
            ...
            if selected_two_time_points[1] in adata_orig.uns["clonal_time_points"]:
                ...
                refine_Tmap_through_joint_optimization(
                    adata,
                    initialized_map,
                    normalization_mode=normalization_mode,
                    sparsity_threshold=sparsity_threshold,
